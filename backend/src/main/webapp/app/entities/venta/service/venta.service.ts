...
type RestOf<T extends IVenta | NewVenta> = Omit<T, 'fechaVenta'> & {
...
export class VentaService {
  ...
  protected convertDateFromClient<T extends IVenta | NewVenta | PartialUpdateVenta>(venta: T): RestOf<T> {
    return {
      ...venta,
      fechaVenta: venta.fechaVenta?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restVenta: RestVenta): IVenta {
    return {
      ...restVenta,
      fechaVenta: restVenta.fechaVenta ? dayjs(restVenta.fechaVenta) : undefined,
    };
  }
